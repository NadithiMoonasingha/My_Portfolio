  ...
  return (
    <section id="skills" style={{ padding: "8rem 2.5rem", maxWidth: "1100px", margin: "0 auto" }}>
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9 }}>
        <p className="text-purple-500 text-sm font-medium tracking-widest uppercase mb-2">Capabilities</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "3rem" }}>
          {Object.entries(portfolioData.skills).map(([category, items]) => (
            <div key={category}>
              <p style={{ fontSize: "11px", letterSpacing: "0.15em", textTransform: "uppercase", color: "#7d489f", marginBottom: "1.25rem" }}>{category}</p>
